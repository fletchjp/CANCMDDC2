...
#define WIFI_THROTTLE 0 // set to 0 if WiFi h/w is not present
...
#define SERIAL_RS485  1  // Provide Serial transmission
...
//#define USE_CODE_SWITCH 1 // Temporary for switch mode in eventhandler. NOT USED ANY MORE
...
// I am defining this to exclude things I think are not needed.
#define NEEDED 0
...
// At the moment BUFFERSIZE is 128 within the file FIFO.h.
// This may need to become something which depends on LINKSPRITE.
#if NEEDED  //FIFO not now used
#include "FIFO.h"
#endif
...
#define RS485PIN 3 // For now - needs checking for clashes
#define DE_PIN RS485PIN
#define RE_PIN RS485PIN
// new Auto485 wrapper using DE_PIN & RE_PIN to toggle read/write mode
// There is a choice here of which serial port pair to use.
...
#define SOUNDER     7
#define SWITCH      3 // was 8  I still have a problem to find a pin.
...
//#endif
  ...
  // configure the module switch, attached to pin SWITCH (was 8), active from GROVE
#if GROVE
  moduleSwitch.setPin(SWITCH, HIGH);
  Serial << F("> Module Switch ") << SWITCH << F(" set to go HIGH when pressed") << endl;
#else
  moduleSwitch.setPin(SWITCH, LOW);
  Serial << F("> Module Switch ") << SWITCH << F(" set to go LOW when pressed") << endl;
#endif 

  // configure the module LED, attached to pin LED via a 1K resistor
  moduleLED.setPin(LED);

#if USE_CBUSBUZZER
  // Set the pin for the buzzer 
  moduleBuzzer.setPin(SOUNDER);
  moduleBuzzer.setTone(TONE);
#if DEBUG
  Serial << F("> Using moduleBuzzer with tone set to ") << TONE << endl;
#endif
#endif
